<template>
  <div class="card">
    ...
    <h6>
      <i>{{ t('components.booksByAuthor.subtitle') }}</i>
    </h6>
...
const exportLabel = ref(t('components.booksByAuthor.labelExport'));
...
const createReport = async () => {
  ...
  if (id === -1) {
    ...
    exportLabel.value = t('components.booksByAuthor.labelExporting');
    await reportService.fetchByAuthor(id);
    exportLabel.value = t('components.booksByAuthor.labelExport');
  }
};
